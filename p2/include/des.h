#ifndef DES_H
#define DES_H
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <string.h>

/* ---------------------- */
/* Constantes para el DES */
/* ---------------------- */

/* Bits salida Permuted Choice 1 */
#define BITS_PC1 56

/* Bits salida Permuted Choice 2*/
#define BITS_PC2 48

/* DES number of rounds*/
#define ROUNDS 16

/* Bits salida Initial Permutation*/
#define BITS_IN_IP 64

/* Bits salida Expansion */
#define BITS_IN_E 48

/* Bits salida permutation after S-Box */
#define BITS_IN_P 32

/* Numero de S-Boxes*/
#define NUM_S_BOXES 8

/* Filas en cada S-Box*/
#define ROWS_PER_SBOX 4

/* Columnas en cada S-Box*/
#define COLUMNS_PER_SBOX 16

/* Tamaño entrada S-Box */
#define SBOX_BLOCK_SIZE 6

/* Variables hexadecimales para facilitar trabajo con la división de la clave */
#define sup56 0xFFFFFFF0000000
#define inf56 0x0000000FFFFFFF
#define sup64 0xFFFFFFFF00000000
#define inf64 0x00000000FFFFFFFF


/* Tabla correspondiente a la Permuted Choice 1 */
static const unsigned short PC1[BITS_PC1] = { 
	57, 49, 41, 33, 25, 17, 9,
	1, 58, 50, 42, 34, 26, 18,
	10, 2, 59, 51, 43, 35, 27,
	19, 11, 3, 60, 52, 44, 36,
	63, 55, 47, 39, 31, 23, 15,
	7, 62, 54, 46, 38, 30, 22,
	14, 6, 61, 53, 45, 37, 29,
	21, 13, 5, 28, 20, 12, 4
};

/* Tabla correspondiente a la Permuted Choice 2 */
static const unsigned short PC2[BITS_PC2] = {
	14, 17, 11, 24, 1, 5,
	3, 28, 15, 6, 21, 10,
	23, 19, 12, 4, 26, 8,
	16, 7, 27, 20, 13, 2,
	41, 52, 31, 37, 47, 55,
	30, 40, 51, 45, 33, 48,
	44, 49, 39, 56, 34, 53,
	46, 42, 50, 36, 29, 32
};

/* Cantidad de bits a rotar en función del número de ronda */
static const unsigned short ROUND_SHIFTS[ROUNDS] = {
	1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1
};

/* Variable contenedora de la permutación conocida como Initial Permutation */
static const unsigned short IP[BITS_IN_IP] = {
	58, 50, 42, 34, 26, 18, 10, 2,
	60, 52, 44, 36, 28, 20, 12, 4,
	62, 54, 46, 38, 30, 22, 14, 6,
	64, 56, 48, 40, 32, 24, 16, 8,
	57, 49, 41, 33, 25, 17, 9, 1,
	59, 51, 43, 35, 27, 19, 11, 3,
	61, 53, 45, 37, 29, 21, 13, 5,
	63, 55, 47, 39, 31, 23, 15, 7
};

/* Tabla de sustitución inversa a la Initial Permutation */
static const unsigned short IP_INV[BITS_IN_IP] = {
	40, 8, 48, 16, 56, 24, 64, 32,
	39, 7, 47, 15, 55, 23, 63, 31,
	38, 6, 46, 14, 54, 22, 62, 30,
	37, 5, 45, 13, 53, 21, 61, 29,
	36, 4, 44, 12, 52, 20, 60, 28,
	35, 3, 43, 11, 51, 19, 59, 27,
	34, 2, 42, 10, 50, 18, 58, 26,
	33, 1, 41, 9, 49, 17, 57, 25
};

/* Tabla de la Expansion Permutation */
static const unsigned short E[BITS_IN_E] = {
	32, 1, 2, 3, 4, 5,
	4, 5, 6, 7, 8, 9,
	8, 9, 10, 11, 12, 13,
	12, 13, 14, 15, 16, 17,
	16, 17, 18, 19, 20, 21,
	20, 21, 22, 23, 24, 25,
	24, 25, 26, 27, 28, 29,
	28, 29, 30, 31, 32, 1
};

/* Tabla de la Permutation tras la sustitución en S-Boxes */
static const unsigned short P[BITS_IN_P] = {
	16, 7, 20, 21,
	29, 12, 28, 17,
	1, 15, 23, 26,
	5, 18, 31, 10,
	2, 8, 24, 14,
	32, 27, 3, 9,
	19, 13, 30, 6,
	22, 11, 4, 25
};

/* Variable contenedora de las 8 S-Boxes para el DES */
static const unsigned short S_BOXES[NUM_S_BOXES][ROWS_PER_SBOX][COLUMNS_PER_SBOX] = {
	{	{ 14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7 },
		{ 0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8 },
		{ 4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0 },
		{ 15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13 } 	},
	{
		{ 15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10 },
		{ 3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5 },
		{ 0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15 },
		{ 13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9 }	},

	{	{ 10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8 },
		{ 13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1 },
		{ 13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7 },
		{ 1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12 }	},

	{	{ 7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15 },
		{ 13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9 },
		{ 10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4 },
		{ 3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14 }	},
	{
		{ 2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9 },
		{ 14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6 },
		{ 4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14 },
		{ 11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3 }	},
	{	
		{ 12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11 },
		{ 10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8 },
		{ 9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6 },
		{ 4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13 }	},
	{
 		{ 4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1 },
		{ 13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6 },
		{ 1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2 },
		{ 6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12 }	},
	{
		{ 13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7 },
		{ 1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2 },
		{ 7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3, 5, 8 },
		{ 2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11 }	}
};

/* ---------------------- */
/*  Funciones para el DES */
/* ---------------------- */

/*
 * Function:  permutation 
 * --------------------
 * 	Permutation generic method for applying a given table permutations
 *
 * 	inpt: Input bits to be operated by substitution boxes.
 * 	table: Table containint pertinent permutations.
 * 	inputsize: Size of Input.
 * 	tablesize: Size of table.
 *
 *  returns: Output bits after each S-Box operation.
 */
uint64_t permutation(uint64_t entrada, const unsigned short *tabla, int sizeEntrada, int sizeTabla);

/*
 * Function:  sbox 
 * --------------------
 * 	S-Box method containing the 8 S-Boxes operations
 *
 * 	inpt: (48b) Input bits to be operated by substitution boxes.
 *
 *  returns: (32b) Output bits after each S-Box operation.
 */
uint64_t sbox(uint64_t entrada);

/*
 * Function:  rotateDES 
 * --------------------
 * 	Rotation method for a given input, size and rotation parameters
 *
 * 	inpt: Input bits to be rotated.
 * 	rotation: Number of bits to rotate.
 * 	size: Size of the performed rotation.
 * 	mask: Mask used in the rotation.
 *
 *  returns: Array of subkeys used in the round method.
 */
uint64_t rotateDES(uint64_t x, int rotation, int size, uint64_t mask);

/*
 * Function:  keyGeneration 
 * --------------------
 * 	Generates all subkeys from the full key in DES algorithm
 *
 * 	fullkey: Key used in this execution.
 *
 *  returns: Array of subkeys used in the round method.
 */
uint64_t* keyGeneration(uint64_t clave);

/*
 * Function:  des 
 * --------------------
 * 	Applies DES algorithm in order to generate a ciphertext
 *
 * 	input: Data input to DES algorithm.
 * 	fullkey: Key used in this execution.
 *
 *  returns: ciphertext encrypted by DES.
 */
uint64_t des(uint64_t M,uint64_t K, int c);

#endif